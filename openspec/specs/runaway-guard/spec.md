...
The runaway guard is the streamed-response volume/degeneracy circuit-breaker
...
transient. This spec covers the **repetition-density** signal; the pre-existing byte
(`MaxDeltaBytes`) and per-block delta-count (`MaxDeltaCount`) budgets are established
context and not restated here.
...
entirely, leaving the byte and delta-count budgets in force. To bound per-request
memory (the ring buffer is allocated per request), `RepetitionWindow` SHALL be
clamped to a fixed maximum so an absurd configured value cannot exhaust memory.
`RepetitionMinUniqueRatio` MUST be in the open interval `(0, 1)`; a value at or
outside that range (`≤ 0` or `≥ 1`) SHALL disable the repetition signal rather than
force a trip on every full window. Configuration SHALL be read at startup; a restart
is required after changing a value.
...
#### Scenario: Out-of-range ratio disables the signal rather than force-tripping

- **WHEN** `RepetitionMinUniqueRatio` is configured at or outside `(0, 1)` — e.g.
  `0`, a negative value, `1`, or a typo like `5` for `0.5`
- **THEN** the repetition signal is disabled (no trip), rather than aborting every
  response once the window fills
