# transactional-self-update Specification

## Purpose

Install an accepted update as a recoverable transaction that either commits a
fully-serving new version or restores the exact previous installation. This
capability owns the minimal RID-specific `copilot-updater` executable, the
immutable policy-free update plan, the frozen cross-executable/cross-release wire
contract, secure HTTPS download + digest verification, safe archive extraction,
template-based `appsettings.json` migration bound to one immutable byte snapshot,
the authenticated two-phase parent/updater handoff, readiness-gated commit,
