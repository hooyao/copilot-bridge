...
found in (`text` or `thinking`), and the action taken (the error signal and
whether it was delivered by injecting a mid-stream event or by buffering). The
record SHALL NOT contain the leaked markup or any tool parameter, envelope child,
or body values.
