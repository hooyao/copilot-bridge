...
[`stream-keepalive`](../stream-keepalive/spec.md) — never reset it. That keeps the
bridge event-gap decision independent from downstream activity. Client watchdogs
remain separate bounds before the first event or whenever keepalives cannot reach
them.
## Requirements
