...
Make the real end-to-end timeout of a long-thinking turn visible at startup, and
warn when the client would abort before the bridge's own budget applies.

Without this the binding bound is effectively invisible: it usually lives in the
*client*, not the bridge, so the bridge logs a healthy `200` while Claude Code
kills the stream — and the operator would otherwise have to derive the answer by
hand from two separate config files, one of which is the client's. Worse, the
bridge itself tightens that client bound as a side effect of the 1M-context env
key it writes, so a default install is misconfigured in a way nothing announces.
