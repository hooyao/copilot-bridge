...
The HybridCache local expiration is the source TTL. The returned entry still carries its authoritative source timestamp, and the resolver checks that timestamp so a record loaded late in its disk lifetime is not granted a fresh full TTL. When an observed memory entry is stale, the resolver calls the same canonical version key with `DisableLocalCacheRead`; HybridCache coordinates that refresh by key and replaces the local value after a cacheable factory result. A stale fallback is surfaced through the shared factory exception to the owner and every coalesced waiter, so the whole request wave observes one outcome without a second revalidation; it is not published into L1, allowing a later request to retry GitHub rather than treating fallback as freshly validated. The process-wide disk-writer `SemaphoreSlim` remains separate and unchanged because it enforces the stronger operator requirement that persistent mutations for different versions never overlap.
