...
#### Scenario: Retry-after duration governs the wait
- **WHEN** a rate-limit response carries a retry-after signal longer than the configured retry spacing
- **THEN** the bridge waits at least the indicated duration before retrying, rather than retrying at its default spacing while the server is still refusing

#### Scenario: Retry-after longer than the remaining budget fails open
- **WHEN** a rate-limit response carries a retry-after duration that would carry the traversal past the overall deadline
- **THEN** the bridge fails open immediately without issuing a further request, rather than retrying before the indicated duration has elapsed
