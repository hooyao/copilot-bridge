...
## 8. PR review follow-ups (Copilot, 3 rounds)

- [x] 8.1 Round 1 — six findings, all correct. Migrated the Playground's two `new AuthService(http)` sites (the CI blocker); replaced the buffered-body regression test after a real-socket probe showed `HttpClient.Timeout` does NOT cover the body read under `ResponseHeadersRead` (it passed for the wrong reason); added the `copilot-metadata` client so `/models` + `count_tokens` keep a bound; detected budgets above the client ceiling; honored the first-party flag; stopped claiming a known effective bound when client settings are unreadable.
- [x] 8.2 Round 2 — three findings. Reverted the round-1 repo-scope "fix": `./.claude/settings.local.json` resolves against the CLAUDE SESSION's project dir, which a bridge serving many repos cannot know, so reading it from the server's CWD could name an unrelated file authoritative. The report now states the gap instead.
- [x] 8.3 Round 3 — the deepest finding: **no finite wall-clock `API_TIMEOUT_MS` can outlast inactivity budgets.** A healthy turn that keeps emitting has no total duration, and a stalled one can spend first-byte + stream-idle gaps before any bridge timer fires (900+600 budgets vs a derived 1200 s). Stopped deriving it; the bridge writes a fixed maximum and the report calls it a residual bound. Spec, README, `docs/timeout-chain.md`, and `appsettings.json` corrected to drop the guarantee they implied.
