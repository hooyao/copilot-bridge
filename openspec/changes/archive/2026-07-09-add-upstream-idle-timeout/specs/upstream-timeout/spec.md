...
The bridge forwards a client request to Copilot and relays the response — a
buffered body or a Server-Sent-Events stream. Copilot can be slow to produce a
first byte (legitimate, for a near-full-context prompt) and can also stall
partway through a stream (a hang). The bridge's only pre-existing backstop is a
coarse total `HttpClient.Timeout` that, under `ResponseHeadersRead`, covers only
the response-headers phase and never the SSE body reads — so a mid-stream stall
is unbounded and even a first-byte stall is bounded far too loosely. This
capability defines an **inactivity (idle) timeout** on the upstream forward path:
a bound on how long the bridge waits *without progress*, distinct from a cap on
total duration, so that a request which keeps making progress is never aborted
however long it legitimately runs.
...
- **Scope:** This capability governs **both** forward paths — the `/cc` Anthropic
  passthrough (`CopilotMessagesPassthroughStrategy`) and the Codex/Responses
  translation path (`CopilotResponsesStrategy`). The first-byte budget is shared at
  the client layer (both `PostMessagesAsync` and `PostResponsesAsync`); the
  stream-idle budget is applied at each strategy's read site. The two paths differ
  only in the mid-stream surface: `/cc` injects a retryable `overloaded_error` (or
  truncates), while Codex flushes a `response.failed` terminal through its existing
  fault channel, because the Codex client speaks the Responses protocol.
