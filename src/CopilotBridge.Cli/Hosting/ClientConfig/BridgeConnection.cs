...
/// <param name="NeedNonStreamingFallbackDisabled">True when either the
/// ResponseLeakGuard or the ToolInputValidation detector runs with
/// <c>Enabled &amp;&amp; PreserveStream</c>. Claude Code only: when true the
/// configurator writes <c>CLAUDE_CODE_DISABLE_NONSTREAMING_FALLBACK=1</c> so a
/// mid-stream detector abort forces a whole-turn retry; when false it removes that
/// env key.</param>
