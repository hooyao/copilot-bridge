...
/// rather than reading raw <c>GetValue&lt;&gt;("Pipeline:Detectors:…")</c> strings —
/// so the two code paths cannot disagree on key names or defaults. Binding uses the
/// source-generated configuration binder (<c>EnableConfigurationBindingGenerator</c>),
/// so it is AOT-clean.
/// </remarks>
internal static class BridgeConnectionFactory
{
    ...
    public static BridgeConnection Create(IConfiguration config, int? cliPort = null)
    {
        ...
        // The fallback env must be written whenever ANY detector can inject an error
        // mid-stream (Enabled && PreserveStream) — that is the exact condition under
        // which Claude Code's silent non-streaming re-request would otherwise swallow
        // the abort instead of retrying the whole turn.
        var needFallback =
