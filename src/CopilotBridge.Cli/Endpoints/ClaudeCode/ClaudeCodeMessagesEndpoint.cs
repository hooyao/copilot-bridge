...
internal static class ClaudeCodeMessagesEndpoint
{
        ...
        ILogger<ClaudeCodeMessagesEndpointTag> endpointLog)
    {
        ...
        // carried into the pipeline (BridgeRequest no longer has a RawBody).
        using var inbound = await InboundBody.ReadPooledAsync(httpCtx.Request.Body, ct).ConfigureAwait(false);
        // Inbound size is a boundary observation — logged here, where it is read,
        // not carried into the pipeline (a body-mutating stage makes it stale).
