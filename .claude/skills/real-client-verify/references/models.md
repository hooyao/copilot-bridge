...
the latest Claude id plus the newest reviewed Codex client identity and resolved gpt
backend, not every historical model. Old
...
2. **Bump the constant** in `ClientBehaviorSupport` once the new id is in the catalog and
   its load-task smoke passes. When the reviewed client catalog already knows the new
   id, bump `LatestGpt`. When compatibility requires retaining an older client identity,
   keep `LatestGpt` on that source and bump `LatestGptBackend`; add a route-specific case
   that records both ids and proves the resolved model from the trace.
