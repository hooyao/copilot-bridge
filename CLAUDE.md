...
- Integration harness (live Copilot + claude.exe): `dotnet test tests/CopilotBridge.Playground` — tagged `[Trait("Category","Integration")]`. New playground tests must carry that trait or CI will try to run them. Routing config reference: `docs/routing.md`.
  - **Two integration sub-kinds, split by what a test proves** (`[Trait("Kind", …)]`): **`Kind=ClientBehavior`** — drives a real `codex.exe`/`claude.exe` through a real bridge subprocess (`ServeProcess`) on the latest models; thin xUnit asserts (harness health only), the semantic verdict is the `real-client-verify` skill's job from the client's own log. **`Kind=ApiContract`** — probes + captured-byte replays that strongly assert exact wire shape (no client driven). Rule: **drives a real client → ClientBehavior; posts bytes / probes an endpoint → ApiContract.** Run the flywheel leg with `dotnet test tests/CopilotBridge.Playground --filter "Kind=ClientBehavior"`.
