...
| 2026-09-07 | Added exact `gpt-6-astra` Responses support and made fresh installs route the reviewed `gpt-5.6-sol` Codex identity to Astra | OpenAI Docs requires Responses for Astra tool calls and recommends moving legacy `none`/`minimal` effort to `low`; live Copilot independently confirmed `none`/`minimal`/`ultra` reject while `low/medium/high/xhigh/max` and current Codex request/tool/event shapes work. Copilot's deployment advertises a lower 1,000,000/872,000 context/prompt budget than the public API's 1,050,000/922,000, so route-aware catalog projection retains gpt-5.6 client instructions but applies Astra capacity and a conservative 850,000 compact threshold. Projection uses target limits only for an invariant first route and hides validated aliases with request-dependent targets or unmappable target limits. A real Codex 0.153.3 `none`-effort run resolved to Astra/low, executed seven matching custom-tool/output rounds, returned the file canary, and recorded zero router/dispatch fatals. Existing installations retain their user-owned Locations array and opt in explicitly. |
