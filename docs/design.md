...
| 2026-08-25 | Updater-managed target and rollback launches prove local serving health without touching credentials or GitHub/Copilot authentication before `Ready`; ordinary launches keep the existing auth gate. | Token expiry, revocation, or a transient refresh failure is external recoverable state, not evidence that the new binary is unhealthy. Gating `Ready` on startup refresh caused a permanent update → auth failure → rollback → retry loop, while pre-commit migration could also delete the only credential format the rollback binary understood. Deferring credential access until the first post-activation request keeps update commit independent from auth and preserves rollback inputs during the readiness window. |
