...
> Status: draft v0.3 · Last updated: 2026-05-08
>
> **2026-05-08 (v0.3)** — Three independent migrations landed atop M1:
> (a) Pipeline framework replaces inline endpoint logic — see
>     [`pipeline-design.md`](pipeline-design.md) for the architectural
>     contract; this doc's §3 architecture diagram now describes intent more
>     than current code shape. (b) Routing config: per-model effort behaviour
>     moved to a C# capability dictionary in `CopilotModelRegistry`;
>     `appsettings.json` keeps only user-preference rules
>     (`Routing.Rules`, Match → Rewrite). See pipeline-design §7. (c) Logger
>     swapped from a bespoke `[Conditional]`-gated tracer to Serilog 4.3.1
>     (console + per-startup file sink) — see pipeline-design §9. The CLI was
>     also rewritten on `System.CommandLine` 3.0-preview.3.
>
...
| 2026-05-08 (v0.3) | Routing split: physical wire facts (effort → variant) live in C# `CopilotModelRegistry.EffortAware`; user preferences (model redirects, effort overrides, thinking quirks) live in `appsettings.json` `Routing.Rules` | Avoids cartesian (model × effort) explosion in JSON; capability table grows linearly with effort-aware models; user-preference rules each express one decision and don't accumulate as boilerplate |
| 2026-05-08 (v0.3) | Serilog 4.3.1 + `Sinks.Console` + `Sinks.File` replaces bespoke `[Conditional]` `DiagTracer` | Standard logger API (everyone recognizes), AOT-clean since Serilog PR #2175, dual sinks (console + per-startup file). Cost: +1.5 MB binary; user explicitly accepted |
| 2026-05-08 (v0.3) | `System.CommandLine` 3.0-preview.3 replaces hand-rolled `string[]` arg parsing | Auto `--help`/`--version`, typed handlers (`int`/`bool` options), nested subcommands; preview is officially AOT-optimized |
