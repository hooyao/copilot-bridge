...
(`bridge_reasoning_origin`) and the Claude edge folds it into the carrier. On replay
a mismatch — the session changed model or route since the carrier was minted — drops
the carrier and runs the turn stateless rather than handing model B a blob only
model A can read. Dropping is right for the same reason established for an
unreplayable item: a turn missing hidden state recovers, a turn carrying foreign
state does not.
