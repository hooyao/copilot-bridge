...
Latest ids under test live in `models.md` (`claude-opus-5-5`; reviewed Codex client
...
> Historical Gate-1 example: the retired opus-5 profile needed a max-effort
> client case to exercise its disabled-thinking × effort clamp. Opus 5.5 rejects
> disabled thinking at every effort, so that case no longer applies. Its
> rewrite-causing rejections are checked on a captured real Claude Code request
> by `Opus55_RealClientCapture_*` and in the live contract sweep.
...
| B5 request + stream retry scopes | deterministic upstream returns one HTTP 500, then reasoning[0] + completed message[1] before a stall that the bridge ends with retryable `response.failed`, then custom `exec` (`Codex_RetryableBridgeStreamTimeout_UsesConfiguredStreamRetries_ForVerdict`) | trace: `500 → reasoning/message → response.failed → custom_tool_call → matching output`; failed `output[0]` keeps message[1]'s added id despite omitted reasoning; isolated `request_max_retries=1`, `stream_max_retries=2`. SQLite: `responses_retry 1/2`, zero fatal/error. Client commands and turn complete with canary. |
...
| B11 updater activation post-Ready auth | B1 through `Codex_UpdaterManagedActivation_ResumesAuthAfterReadyAndCompletesToolChain_ForVerdict`, with a fake updater pipe and complete target activation context | fake updater receives a valid target `Ready`; before the client starts, the bridge must complete deferred residual cleanup + direct version-2 lease resolution. Trace/stdout/SQLite: complete tool round-trip, final canary, no abort, zero router/dispatch fatal. |
...
| B14 custom OAuth v4 forced first CAPI 403 | B6 through `Codex_CustomOAuthVersionFour_OneShotCopilot403_RotatesAndCompletesComplexToolChain_ForVerdict`, staging a separately authorized, marker-validated, single-use refreshable version-4 credential from `COPILOT_BRIDGE_TEST_CUSTOM_RECOVERY_CREDENTIAL_SOURCE_DIRECTORY` | bridge lifecycle: one injected 403, a forced version-4 OAuth rotation through its recorded client ID, one successful direct-lease authentication replay, and no terminal policy classification. Trace/stdout/SQLite: matching tool outputs, completed canary, no abort, zero router/dispatch fatal. Never reuse the B13/installed credential source: GitHub invalidates the copied rotating token pair. |
...
| B19 Astra attached image | `CodexVisionBehaviorTests.Codex_Gpt56RoutedToAstra_AttachedImage_ProducesVisionEvidenceForVerdict` attaches a fresh PNG through native app-server `localImage`, then requires native `apply_patch` and a separate shell read | first inference contains the image; every upstream request is Astra/low; image-bearing upstream requests carry `copilot-vision-request=true`. Saved file and final client answer match the image-only six-digit code and all four shuffled color positions. Matching write/read tool outputs, completed client turn, no abort, nonempty SQLite with zero router/dispatch fatal or ERROR rows. See the manifest's `visionEvidencePath`. |
| B20 Astra image tool | `CodexVisionBehaviorTests.Codex_Gpt56RoutedToAstra_ViewImageTool_ProducesVisionEvidenceForVerdict` starts with text only, requires real `view_image` on a generated PNG, then native `apply_patch` and a separate shell read | no image in the first inference; client evidence shows actual `view_image` execution and an image-bearing continuation (including its enclosing exec output when applicable). Apply B19's routing, exact visual answer, tool-loop, and client-log checks. Prompt/filename do not reveal the expected answer. |
...
Client `claude.exe`, scenario `CcToGpt` (promotes the `claude-opus-5.5 → gpt-5.6-sol`
location), route `/cc->gpt`. The client speaks `claude-opus-5-5`; routing rewrites it
